...
const projects = [
  ...
  },
  {
    ...
    star: {
      situation: "基于 Olist 公开数据集（9 张表、9.8 万订单、10 万条评论）分析平台 13.7% 差评率的成因，需先做数据治理，修正三处口径陷阱：复购率误算为 0.00%、同日拆单虚高 29%、JOIN 使 GMV 虚高 4.57%。",
      task: "验证配送延迟对差评是否存在因果效应，区分责任在卖家还是物流网络，并判断能否在下单时就预警高风险订单。",
      action: "用倾向得分匹配（110 维协变量，匹配后 SMD < 0.045）估计延迟对差评的因果效应，经 IPW 与回归调整交叉验证（四法极差 0.67pp）；将延迟责任拆解至运输段（85.5%），论证问题在物流网络而非卖家治理；构建下单时预警模型，排查修复了一处时间穿越泄漏（AUC 由虚高的 0.932 回落至真实的 0.636，归因结论随之反转）。",
      result: "ATT +45.05pp；预警模型 TOP 10% 订单的差评概率提升 2.43 倍；测算延迟率从 8.0% 降到 5.0% 可消除 10.5% 的差评。",
    },
    tone: "steel",
    preview: "metric",
